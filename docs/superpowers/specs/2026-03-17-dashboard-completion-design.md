...
Complete the OpenNeuro Dashboard with pipeline stages 4-5, code quality fixes, and CI/CD scheduling.

## Context

The dashboard monitors synchronization of OpenNeuro datasets across GraphQL API, GitHub mirrors, and S3 exports. Pipeline stages 1-3 and 6 are implemented. Stages 4-5 (git file trees and S3 file diffs) exist only as test data generators. The frontend is complete but expects data from unimplemented stages and lacks error boundaries.
...
## Stage 4: `fetch_git_tree.py`

Clone bare git repos into a persistent local cache. No JSON output — the bare repo is the artifact.
...
1. Load registry and S3 version results.
2. For each dataset where S3 is accessible (cases 1 and 2 from stage 3):
   ...
   - **Tag already present:** skip
3. Skip datasets where S3 is blocked (case 3, HTTP 403) or not found (case 4) — no file diff is meaningful without S3 access.
4. Concurrency: asyncio semaphore, default 10 concurrent clones.

### Output

Bare repos at `{cache_dir}/{id}.git`. Default cache directory: `~/.cache/openneuro-dashboard/repos`.

### CLI

- `--cache-dir` — override default cache location
- `--output-dir` — data directory (default `data/`)
- `--validate` — verify each expected tag is reachable in the cached repo

## Stage 5: `check_s3_files.py`

Compare S3 file listing against git tree at the resolved tag. Write a unified file diff with context.

### Inputs

- `datasets/{id}/s3-version.json` — resolved tag and accessibility
- Cached bare repos from stage 4

### Logic

1. For each dataset with a cached bare repo and accessible S3:
   - Open bare repo with `pygit2`, resolve tag, walk tree to build a set of file paths.
   - List S3 objects under `{id}/` prefix using unsigned `aioboto3` requests. Strip prefix. Build a set.
   - Compute diff:
     - `added` = git files not in S3 (S3 needs these)
     - `removed` = S3 files not in git (orphaned in S3)
   - Compute context: sort all git files. For each file in `added` or `removed`, find its position in the sorted list and collect up to 3 files on each side. Context = union of all neighbors minus the added/removed files themselves.
2. Incremental: skip datasets where S3 version hasn't changed since last diff (compare version in `s3-version.json` against stored version in `s3-diff.json`).
...
- `--cache-dir` — bare repo cache location
- `--output-dir` — data directory (default `data/`)
- `--validate` — verify output consistency

### Concurrency

- Asyncio semaphore (~20) for S3 list requests
...
4. Run pipeline stages sequentially:
   ...
   - `uv run scripts/fetch_git_tree.py --cache-dir ~/.cache/openneuro-dashboard/repos`

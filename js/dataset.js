...
async function init() {
  ...
  try {
    ...
    // Show diff summary above the fold if diff data exists
    if (s3Diff && !s3Diff.exportMissing) {
      const addedCount = s3Diff.added ? s3Diff.added.length : 0;
      const removedCount = s3Diff.removed ? s3Diff.removed.length : 0;
      document.getElementById("detail-files-added").textContent =
        addedCount.toLocaleString();
      document.getElementById("detail-files-removed").textContent =
        removedCount.toLocaleString();
      document.getElementById("detail-git-total").textContent =
        s3Diff.totalGitFiles.toLocaleString();
      document.getElementById("detail-s3-total").textContent =
        s3Diff.totalS3Files.toLocaleString();
      document.getElementById("diff-summary").style.display = "block";
      if (addedCount > 0 || removedCount > 0) {
        document.getElementById("diff-summary").style.borderColor =
          "var(--status-error)";
      }
    } else if (s3Diff && s3Diff.exportMissing) {
      document.getElementById("detail-files-added").textContent = "N/A";
      document.getElementById("detail-files-removed").textContent = "N/A";
      document.getElementById("detail-git-total").textContent =
        s3Diff.totalGitFiles.toLocaleString();
      document.getElementById("detail-s3-total").textContent = "0";
      document.getElementById("diff-summary").style.display = "block";
      document.getElementById("diff-summary").style.borderColor =
        "var(--status-error)";
    }
...
function renderSnapshots() {
  ...
  Promise.all(
    ...
    container.innerHTML = snapshotData
      .reverse()
      .map(
        (snap) => `
...
function renderGitHubCheck() {
  ...
  document.getElementById("github-branches").innerHTML = Object.entries(
    github.branches,
  )
...
function getS3VersionStatus() {
  ...
  return s3Version.extractedVersion === latestSnapshot
    ? "ok"
    : "version-mismatch";
}
...
function renderS3VersionCheck() {
  ...
  if (!s3Version) {
    ...
    document.getElementById("s3-version-summary").textContent =
      "Check not yet run";
  ...
  if (!s3Version.accessible) {
    ...
    const summary =
      `Access to S3 files is forbidden (HTTP 403). The dataset is marked as ` +
  ...
  icon.textContent =
    status === "ok" ? "✓" : status === "version-mismatch" ? "≠" : "⚠";
  icon.className = `check-icon-large ${status}`;
  ...
  // Details
  document.getElementById("s3-doi").textContent =
    s3Version.datasetDescriptionDOI || "N/A";
  document.getElementById("s3-extracted-version").textContent =
    extractedVersion;
  document.getElementById("s3-expected-version").textContent = latestSnapshot;
  document.getElementById("s3-version-last-checked").textContent = formatDate(
    s3Version.lastChecked,
...
function renderS3FilesCheck() {
  ...
  if (!s3Version) {
    document.getElementById("s3-files-summary").textContent =
      "S3 version check not yet run";
    return;
  }

  if (!s3Diff) {
    document.getElementById("s3-files-summary").textContent =
      "File comparison not yet run";
    return;
  }

  if (s3Diff.exportMissing) {
    const summary =
      `No files found on S3 (0/${s3Diff.totalGitFiles}). ` +
  ...
  if (status === "ok") {
    ...
    summary =
      `✗ File mismatch: ${addedCount} file${addedCount !== 1 ? "s" : ""} missing from S3, ` +

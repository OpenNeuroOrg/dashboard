#!/usr/bin/env python3
"""
Stage 1: Simulate GraphQL fetch.
...
from utils import (
    SCHEMA_VERSION, random_datetime, random_sha, generate_dataset_id,
    generate_snapshots, get_latest_snapshot, write_json
)


def generate_graphql_data(output_dir: Path, num_datasets: int, seed: int = None):
...
def main():
    parser = argparse.ArgumentParser(description="Generate simulated GraphQL data")
    parser.add_argument("--output-dir", type=Path, default=Path("data"))
    parser.add_argument("--num-datasets", type=int, default=20)
    parser.add_argument("--seed", type=int, help="Random seed for reproducibility")
    args = parser.parse_args()
    
    generate_graphql_data(args.output_dir, args.num_datasets, args.seed)
